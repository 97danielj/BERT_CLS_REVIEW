    ...
    while True: # refresh 반복뮨
        while True: # 매장 접근을 위한 반복문
            ...
            # 예상치 못한 오류가 뜨는 경우
                ...
                # json파일 저장
                with open(f'./naver_review_with_rank/naver_{key_word}_review_dic_page{page_num + 1}.json', 'w', encoding='utf-8') as f:
...
key_word_list_ex = [ '부산 횟집', '대구 횟집', '광주 횟집', '대전 횟집', '울산 횟집']
key_word_list = ['서울 횟집','인천 횟집', '부산 횟집', '대구 횟집', '광주 횟집', '대전 횟집', '울산 횟집']

naver_crawling(key_word_list[1],1,19)
for page_num in range(1,2):
    for key_word in key_word_list_ex:
